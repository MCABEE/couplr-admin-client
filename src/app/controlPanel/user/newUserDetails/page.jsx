...
const NewUserProfile = () => {
  ...
  const [selectedValue, setSelectedValue] = useState(1); // Default 1 for minimum progress
  ...
  const handleVerify = async (status) => {
    ...
    const confirmAction = window.confirm(
      `Are you sure you want to ${status.toLowerCase()} this ID proof?`
    );
  ...
  useEffect(() => {
    const percentageString =
      selectedUser?.profileCompletion?.progressPercentage;
    if (percentageString) {
      const numericValue = parseFloat(percentageString); // "5.21%" => 5.21
      const rounded = Math.round(numericValue); // Round to nearest whole number
      // const stepValue = Math.min(Math.ceil((rounded / 100) * 10), 10); // Convert 0-100% to 1-6 scale
  ...
  return (
    <div className="w-full bg-[#263238] py-5 relative px-4">
      <button className="absolute -top-3 -right-2 bg-white rounded-full p-1">
        <Link href={"/controlPanel/user/newSignup"}>
          <img
            src="/closeButton.png"
            alt="Close"
                    ...
                    <span
                      ...
                      className={`cursor-pointer ${
                        index + 1 === selectedValue
                          ? "text-black font-bold text-[8px]"
                          : " text-[8px] text-[#202020]"
                      }`}
