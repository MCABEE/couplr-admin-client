...
// const Table = () => {
//   const { newUsers, isLoading, fetchActiveUsers, totalPages } = useUserStore();
...
const Table = () => {
  const { activeUsers, isLoading, fetchActiveUsers, totalPages } = useUserStore(); // ✅ Use activeUsers
  const [groupedUsers, setGroupedUsers] = useState({});
  const [currentPage, setCurrentPage] = useState(1);

  useEffect(() => {
    fetchActiveUsers(currentPage); // ✅ Fetch active users
  }, [currentPage]);

  useEffect(() => {
    if (activeUsers.length > 0) { // ✅ Use activeUsers instead of newUsers
  ...
  }, [activeUsers]); // ✅ Update based on activeUsers
          ...
          <table className="w-full border-spacing-y-2 border-separate mt-10 text-black">
            <thead className="bg-[#C0D8F6]">
              <tr className="p-2 bg-[#C0D8F6]">
                ...
                <th className="text-base border-l-4 border-[#F0F4F9] p-2">Current Location</th>
                <th className="text-base border-l-4 border-[#F0F4F9] p-2">Gender</th>
                <th className="text-base border-l-4 border-[#F0F4F9] p-2">Active Status</th>
                <th className="text-base border-l-4 border-[#F0F4F9] p-2 rounded-r-2xl">Acc. Vintage</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(groupedUsers).map(([date, users]) => (
                <React.Fragment key={date}>
                  ...
                  {users.map((user, index) => (
                    <tr key={user._id} className="bg-white">
                      <td className="p-2">{index + 1}</td>
                      <td className="border-l-4 border-[#C0D8F6] p-2">{user.displayName}</td>
                      <td className="border-l-4 border-[#C0D8F6] p-2">
                        {user.currentLocation?.city || "N/A"}, {user.currentLocation?.country || "N/A"}
                      </td>
                      <td className="border-l-4 border-[#C0D8F6] p-2">{user.gender || "N/A"}</td>
                      <td className="border-x-4 border-[#C0D8F6] p-2">
                        {user.activeStatus || "0"} d
                      </td>
                      <td className="p-2">{user.accountVintage || "N/A"}</td>
                    </tr>
                  ))}
                </React.Fragment>
              ))}
